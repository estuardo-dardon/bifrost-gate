...
#[cfg(target_os = "linux")]
pub fn get_swanctl_base_dir_pub() -> String {
    // Permitir configuración vía variable de entorno
...
#[cfg(target_os = "linux")]
pub async fn with_swanctl_lock_pub<T, F>(f: F) -> Result<T, String>
where
...
#[cfg(target_os = "linux")]
pub async fn write_file_atomic_with_backup_pub(
    path: &PathBuf,
    contents: &str,
    perms: Option<u32>,
...
#[cfg(target_os = "linux")]
pub async fn restore_backup_or_delete_pub(path: &PathBuf, backup: Option<String>) -> Result<(), String> {
    match backup {
...
// ─── Aliases privadas (uso interno del módulo) ────────────────────────────────

#[cfg(target_os = "linux")]
#[inline(always)]
fn get_swanctl_base_dir() -> String {
    get_swanctl_base_dir_pub()
}
...
async fn write_file_atomic_with_backup(
    ...
    write_file_atomic_with_backup_pub(path, contents, perms).await
}

#[cfg(target_os = "linux")]
#[inline(always)]
async fn restore_backup_or_delete(path: &PathBuf, backup: Option<String>) -> Result<(), String> {
    restore_backup_or_delete_pub(path, backup).await
}
